...
from dilu.driver_agent.model_provider import setup_model_env
...
if __name__ == '__main__':
    ...
    auto_save_memory = config.get("auto_save_memory", False)  # 默认手动模式
                    ...
                    for i in range(collision_frame, -1, -1):
                        if docs[i]["action"] != 4:  # not decelearate
                            ...
                            if auto_save_memory:  # 自动保存模式
                                ...
                                print("[green] Auto-saved corrected memory to update memory module.[/green] Now the database has ", len(
                    ...
                    if auto_save_memory:  # 自动保存模式
                        ...
                        for i in range(0, len(docs)):
                            if i % 5 == 1:
                                ...
                                cnt +=1
                        print("[green] Auto-saved[/green] ",cnt," [green]new memory items to update memory module.[/green]. Now the database has ", len(
                                    updated_memory.scenario_memory._collection.get(include=['embeddings'])['embeddings']), " items.")
                    else:  # 手动确认模式
